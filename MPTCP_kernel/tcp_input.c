...
static void tcp_rtt_estimator(struct sock *sk, long mrtt_us)
{
	...
	tp->srtt_us = max(1U, srtt);
	 printk("TCPcong: tcp_input   tcp_rtt_estm           %14u %19p\n",  tp->srtt_us, tp);
}
				      ...
				      long seq_rtt_us, long sack_rtt_us)
{
	...
	if (seq_rtt_us < 0 && tp->rx_opt.saw_tstamp && tp->rx_opt.rcv_tsecr &&
	    flag & FLAG_ACKED)
//	        printk("TCPcong: tcp_input   tcp_ack_up_j           %14u %10u %p\n", tcp_time_stamp, tp->rx_opt.rcv_tsecr, tp);
		seq_rtt_us = jiffies_to_usecs(tcp_time_stamp - tp->rx_opt.rcv_tsecr);

//        printk("TCPcong: tcp_input   tcp_ack_up_a           %14u %10u %p\n", tcp_time_stamp, seq_rtt_us, tp);
...
//        printk("TCPcong: tcp_input   tcp_ack_up tsval tsecr %14u %10u %p\n", tp->rx_opt.rcv_tsval, tp->rx_opt.rcv_tsecr, tp);

        /*call_delivery_delay_estimator*/
        
        tcp_delivery_delay_estimator(sk);
        
        /*call_farid_srtt_f_estimator*/
	
//	tcp_srtt_f_estimator(sk);
...
//					printk("TCPcong: tcp_input   tcp_par_o tsval tsecr  %14u %10u %p\n",  opt_rx->rcv_tsval, opt_rx->rcv_tsecr, opt_rx);
				}
				break;
...
/*
void tcp_srtt_f_estimator(struct sock *sk)
{       
...
/*                        
        ...
        u32 srtt_f_new;
        u32 srtt_f_old = tp->srtt_f;
        
        
        printk("TCPcong: tcp_input   srtt_f-es  tstm  tsecr %14u %10u %p\n", tcp_time_stamp, tp->rx_opt.rcv_tsecr, tp);

        srtt_f_new = tp->rx_opt.rcv_tsecr;
        
        printk("TCPcong: tcp_input   d-delay-es_x srtt_f_c  %14u %19p\n",  srtt_f_new, tp);                                                                                                        



         if (srtt_f_old !=0){
                     tp->srtt_f = srtt_f_old - (srtt_f_old >> 3) + (srtt_f_new >> 3); /* 7/8 * fw_delay_old + 1/8 * fw_delay_new *//*
//                   printk("TCPcong: tcp_input   d-delay-es_d fw_delay  %14u %19p\n",  tp->fw_delay, tp);
//                   printk("TCPcong: tcp_input   d-delay-es_e fw_d_old  %14u %19p\n",  fw_delay_old, tp);
         }
         else{
                     tp->srtt_f = srtt_f_new;
//		     printk("TCPcong: tcp_input   smooth      srtt_f    %14u %19p\n",  tp->srtt_f, tp);  
          }
...
void tcp_delivery_delay_estimator(struct sock *sk)
{
        
        /*delivery_delay_estimator*/
        
        struct tcp_sock *tp = tcp_sk(sk);
        
       // long  fw_delay_new ;
       // long fw_delay_new_j;
       // u32 fw_delay_old = tp->fw_delay;                                                                                                                           
         
        u32 fw_delay_new;
        u32 fw_delay_old = tp->fw_delay;
         
        printk("TCPcong: tcp_input   d-delay-es tsval tsecr %14u %10u %p\n", tp->rx_opt.rcv_tsval, tp->rx_opt.rcv_tsecr, tp);
                
        
        if (tp->rx_opt.rcv_tsval > tp->rx_opt.rcv_tsecr)  
        {
                fw_delay_new = tp->rx_opt.rcv_tsval - tp->rx_opt.rcv_tsecr;
//                printk("TCPcong: tcp_input   d-delay-es_x fw_del_c  %14u %19p\n",  fw_delay_new, tp);
        }
        else
        {
                fw_delay_new = tp->rx_opt.rcv_tsecr - tp->rx_opt.rcv_tsval ;
//                printk("TCPcong: tcp_input   d-delay-es_y fw_del_e  %14u %19p\n",  fw_delay_new, tp);
        }            
        
          
          /*fw_delay_new_j = jiffies_to_usecs(tp->rx_opt.rcv_tsval - tp->rx_opt.rcv_tsecr);
          printk("TCPcong: tcp_input   d-delay-es_b fw_del_j  %14ld %19p\n",  fw_delay_new_j, tp);
          */
          
/*          fw_delay_new = tp->rx_opt.rcv_tsval - tp->rx_opt.rcv_tsecr;
          printk("TCPcong: tcp_input   d-delay-es_j fw_del_n  %14u  %19p\n",  fw_delay_new, tp);
          
          
          if(fw_delay_new < 0) 
          {
                 fw_delay_new = - fw_delay_new;
                  printk("TCPcong: tcp_input   d-delay-es_c fw_del_p  %14u %19p\n",  fw_delay_new, tp);
          }
*/          
          
        /*Smothing the delivery delay*/
          
        if (fw_delay_old !=0){
                tp->fw_delay = fw_delay_old - (fw_delay_old >> 3) + (fw_delay_new >> 3); /* 7/8 * fw_delay_old + 1/8 * fw_delay_new */
//              printk("TCPcong: tcp_input   d-delay-es_d fw_delay  %14u %19p\n",  tp->fw_delay, tp);
//              printk("TCPcong: tcp_input   d-delay-es_e fw_d_old  %14u %19p\n",  fw_delay_old, tp);
        }
        else{
                tp->fw_delay = fw_delay_new;
//      printk("TCPcong: tcp_input   smooth      fw_delay  %14u %19p\n",  tp->fw_delay, tp);  
        }
        printk("TCPcong: tcp_input   smooth       fw_delay  %14u %19p\n",  tp->fw_delay, tp);
        ...
        srtt_f_new = tcp_time_stamp - tp->rx_opt.rcv_tsecr;
...
//                   printk("TCPcong: tcp_input   smooth      srtt_f    %14u %19p\n",  tp->srtt_f, tp);  
        ...
        /*backward_delay_estimation*/

        u32 bw_delay_new;
        u32 bw_delay_old = tp->bw_delay;
        
        printk("TCPcong: tcp_input   srtt_f-es  tstm  tsval %14u %10u %p\n", tcp_time_stamp, tp->rx_opt.rcv_tsval, tp);
                        
//        bw_delay_new = tcp_time_stamp - tp->rx_opt.rcv_tsval;

        if (tcp_time_stamp > tp->rx_opt.rcv_tsval)
        {
                bw_delay_new = tcp_time_stamp - tp->rx_opt.rcv_tsval;
//                printk("TCPcong: tcp_input   d-delay-es_x bw_del_c  %14u %19p\n",  fw_delay_new, tp);
        }   
        else
        {
                bw_delay_new = tp->rx_opt.rcv_tsval - tcp_time_stamp ;
//                printk("TCPcong: tcp_input   d-delay-es_y bw_del_e  %14u %19p\n",  fw_delay_new, tp);
        } 
                                                                                    

        printk("TCPcong: tcp_input   d-delay-es_x bw_delay  %14u %19p\n",  bw_delay_new, tp);
        
        if (bw_delay_old !=0){                                    
                tp->bw_delay = bw_delay_old - (bw_delay_old >> 3) + (bw_delay_new >> 3); /* 7/8 * bw_delay_old + 1/8 * bw_delay_new */
//                printk("TCPcong: tcp_input   d-delay-es_d bw_delay  %14u %19p\n",  tp->bw_delay, tp);
//                printk("TCPcong: tcp_input   d-delay-es_e bw_d_old  %14u %19p\n",  bw_delay_old, tp);
        }
        else{
                tp->bw_delay = bw_delay_new;
//                printk("TCPcong: tcp_input   smooth      bw_del    %14u %19p\n",  tp->bw_delay, tp);  
        }
